--[[
    ...
    Title screen displaying title and start/exit options
]]
    ...
    for y = 1, 8 do
        for x = 1, 8 do
            ...
            love.graphics.draw(gTextures['main'], positions[(y - 1) * x + x], 
                (x - 1) * 70 + 128 + 3, (y - 1) * 90 + 16 + 3)

            -- render tile
            love.graphics.setColor(255, 255, 255, 255)
            love.graphics.draw(gTextures['main'], positions[(y - 1) * x + x], 
                (x - 1) * 70 + 128, (y - 1) * 90 + 16)
        end
    end
    ]]
...
function StartState:drawMatch3Text(y)
    ...
    love.graphics.rectangle('fill', VIRTUAL_WIDTH / 2 - 100, VIRTUAL_HEIGHT / 2 + y - 11, 200, 58, 6)

    -- draw MATCH 3 text shadows
    love.graphics.setFont(gFonts['large'])
    self:drawTextShadow('NUTCRACKER', VIRTUAL_HEIGHT / 2 + y)
    ...
    love.graphics.setColor(255, 255, 255, 255)
    love.graphics.draw(gTextures['title'], 150, 140)
end
...
function StartState:drawOptions(y)
    ...
    --draw Start text
    love.graphics.setFont(gFonts['textlarge'])
    if self.currentMenuItem == 1 then
    ...
    love.graphics.printf('Start', 0, VIRTUAL_HEIGHT / 2 + y, VIRTUAL_WIDTH, 'center')
    ...
    love.graphics.printf('Quit', 0, VIRTUAL_HEIGHT / 2 + y + 60, VIRTUAL_WIDTH, 'center')
end
